...
def get_civitai_data(model_url):
    try:
        url = model_url.split('?token=')[0] if '?token=' in model_url else model_url
        ...
        if 'civitai.com/api/download/models/' in url:
            # data é da model-version
            target_version = data
            model_id = str(data.get('modelId', ''))
        else:
            # data é do model ou model-version
            if versionId:
                target_version = data
                model_id = str(data.get('modelId', ''))
            else:
                # data é do model, pegar primeira versão
...
def civ_download(url, dest_path, category=None, retries=3):
    # Extrai metadados antes de baixar se for Civitai e categoria apropriada
    version_data = None
    metadata = None
    
    # Verifica se é um link do Civitai
    if "civitai.com" in url:
        model_id, version_data, metadata, download_url = get_civitai_data(url)
    ...
    if version_data:
        # Para Civitai, usa o nome da API
        files = version_data.get("files", [])
        primary_file = next((f for f in files if f.get("primary")), files[0] if files else {})
        filename = primary_file.get("name", "downloaded_file")
    else:
        # Para outros, faz requisição
    ...
    for attempt in range(retries):
        try:
            ...
            linhas = []
            padrao = re.compile(r'\[(#[a-z0-9]+ \d+MiB/\d+MiB\(\d+%\) CN:\d+ DL:\d+MiB(?: ETA:\d+s)?)\]')
            ultima_info = None
            ...
            for linha in process.stdout:
                ...
                if match:
                    ...
                    if info_progresso != ultima_info:
                        # Usa ANSI escape para limpar a linha e atualizar progresso
                        sys.stdout.write(f"\r\033[{info_progresso}]")
                        sys.stdout.flush()
            ...
            if process.returncode != 0:
                raise Exception(f"Erro no aria2c: código {process.returncode}")
...
def unzipfiles(zip_path, extract_to):
    try:
        ...
        for url in urls:
            ...
            if result:
                ...
                if (category in UNZIP_CATEGORIES) and downloaded_file.endswith(".zip"):
                    unzipfiles(downloaded_file, dest_dir)
                    zip_extractions += 1
