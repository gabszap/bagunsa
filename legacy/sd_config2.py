...
# Resolve URL do Civitai para link de download direto da API
def resolve_civitai_url(model_url):
    try:
...
def update_hub_config(config_path):
    try:
...
def main():
    ...
    for category, urls in downloads.items():
        ...
        for url in urls:
            ...
            if downloaded_file and (category in UNZIP_CATEGORIES) and downloaded_file.endswith(".zip"):
                unzip_and_remove(downloaded_file, dest_dir)
            # Se baixou config do hub, atualiza tokens
            if downloaded_file and category == "hub":
                base = os.path.basename(downloaded_file)
                if base == "sd-hub-config.json":
                    update_hub_config(downloaded_file)
